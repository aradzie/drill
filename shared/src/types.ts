...
/** Removes top-level readonly modifiers, including on arrays and tuples, while preserving primitives. */
export type Mutable<T> = T extends Primitive ? T : { -readonly [K in keyof T]: T[K] };

/** Recursively makes plain data mutable, including arrays and tuples, while preserving primitives. */
export type DeepMutable<T> = T extends Primitive ? T : { -readonly [K in keyof T]: DeepMutable<T[K]> };

/** Recursively makes plain data readonly, including arrays and tuples. */
